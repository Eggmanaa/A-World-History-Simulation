...
export const ArchimedesTower3D: React.FC<{ position: [number, number, number] }> = ({ position }) => {
    return (
        <group position={position}>
            {/* Tall cylinder base with sphere on top */}
            <mesh position={[0, 0.5, 0]} castShadow>
                <cylinderGeometry args={[0.25, 0.3, 0.9, 8]} />
                <meshStandardMaterial color="#64748b" roughness={0.7} /> {/* Slate grey stone */}
            </mesh>
            {/* Top sphere beacon */}
            <mesh position={[0, 1.15, 0]} castShadow>
                <sphereGeometry args={[0.25, 16, 16]} />
                <meshStandardMaterial color="#fbbf24" metalness={0.6} roughness={0.2} /> {/* Golden beacon */}
            </mesh>
        </group>
    );
};
