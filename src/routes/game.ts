...
game.post('/build', async (c) => {
  try {
    ...
    if (buildingType === 'house') {
      // Houses cost 0 industry and are limited by fertility per turn
      // Check if already built houses this turn equal to fertility
      if ((civData.houses_built_this_turn || 0) >= civData.fertility) {
        return c.json({ 
          error: `Can only build ${civData.fertility} houses per advancement (limited by fertility)` 
        }, 400)
      }
